-- ============================================================
-- COMPLETE PRODUCTION FIX - RUN THIS ENTIRE SCRIPT
-- This fixes schema issues AND adds all data
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added allow_short_selling column';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added max_trades_per_team column';
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        RAISE NOTICE 'Added trading_fee_percentage column';
    END IF;
    
    -- Remove status column if it exists (it's a @property in Django, not a DB field)
...
SELECT 'Events:' as info, COUNT(*) as count FROM app1_event
UNION ALL SELECT 'Settings:', COUNT(*) FROM app1_simulatorsettings  
UNION ALL SELECT 'Stocks:', COUNT(*) FROM app1_stock
UNION ALL SELECT 'Active Stocks:', COUNT(*) FROM app1_stock WHERE is_active = true;

-- ============================================================
-- 🎉 SUCCESS! Database is now fully initialized!
-- 
-- You should see:
-- - 1 Event
-- - 2 Settings  
...
--
-- Next steps:
-- 1. Refresh your admin page - all errors should be gone!
-- 2. Go to /team/signup to create a team
-- 3. Go to /team/stocks to see all 63 stocks
-- ============================================================
